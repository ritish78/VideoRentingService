    ...
    public partial class AdminLogIn : System.Web.UI.Page
    {
        ...
        protected void btn_LogIn_Click(object sender, EventArgs e)
        {
            using(SqlConnection sqlConnection = new SqlConnection(ConfigurationManager.ConnectionStrings["ConnectionStringCustomerInfo"].ConnectionString))
            {
                ...
                string query = "SELECT COUNT(1) FROM adminInfo WHERE email=@email AND passwd=@passwd";
                ...
                if (count == 1)
                {
                    string queryName = "SELECT firstName FROM adminInfo WHERE email=@email";
                    string queryLast = "SELECT lastName FROM adminInfo WHERE email=@email";
                    string queryPhoneNo = "SELECT phoneNumber FROM adminInfo WHERE email=@email";
                    string queryDOB = "SELECT dateOfBirth FROM adminInfo WHERE email=@email";
                    ...
                    HttpCookie cookie = new HttpCookie("AdminDetails");
